...
* Переходим в каталог `selinux_dns_problems` и выполняем старт ВМ: `vagrant up`

* Выполяем вход в ВМ client: `vagrant ssh client`

```bash
$ vagrant ssh client
Last login: Wed Dec 28 12:02:02 2022 from 10.0.2.2
###############################
...
* Пробуем внести изменения в зону: `nsupdate -k /etc/named.zonetransfer.key`
...
* Изменения внести не получилось, смотрим логи `SELinux` чтобы понять в чём может быть проблема:
...
```bash
$ vagrant ssh ns01
Last login: Wed Dec 28 11:56:30 2022 from 10.0.2.2
[vagrant@ns01 ~]$ sudo -i
[root@ns01 ~]# cat /var/log/audit/audit.log | audit2why 
type=AVC msg=audit(1672232677.972:1917): avc:  denied  { create } for  pid=5068 comm="isc-worker0000" name="named.ddns.lab.view1.jnl" scontext=system_u:system_r:named_t:s0 tcontext=system_u:object_r:etc_t:s0 tclass=file permissive=0
...
* Возможных решений два: первое, это перенести файлы зон в каталог `/var/named/` и изменить в конфигурации `/etc/named.conf` их расположение, второе, можно изменить тип контекста безопаности для каталога `/etc/named`.

* По моему мнению лучше переместить каталог зоны на место где он должен быть (`/var/named`), и не менять контекст безопасности на каталог `/etc/named`.

* Переместим все файлы и каталоги из `/etc/named` в `/var/named`: `rsync -a /etc/named/ /var/named/ && rm -rf /etc/named/*`

* Восстановим контекст безопасности рекурсивно в каталоге `/var/named`: `restorecon -R /var/named`
...
* Перечитаем файл конфигурации сервиса `named`: `systemctl reload named`
